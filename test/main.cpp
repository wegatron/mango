...
int exportFvsObj2()
{
  ...
  for (auto file_id = 0; file_id < 101; ++file_id) {
    #if EXPORT_ORDER
    std::string file_path =
        "/home/wegatron/opt/blender-3.6.1-linux-x64/3.6/scripts/addons/mh_anim/data/mh_template.fbx";
    #else
    std::string file_path =
        "/home/wegatron/win-data/data/FLAME/mh_shapes/flame_" +
        std::to_string(file_id) + ".fbx";    
      ...
      std::sort(keys.begin(), keys.end(), [mesh_i](const Eigen::Vector4i &a, const Eigen::Vector4i &b) {
        if(a.x() == b.x())
        {
          if(a.y() == b.y())
          {
            // assert(false);
            // if (a.z() == b.z()) {
            //   std::cout << "mesh_i:" << mesh_i << a.transpose() << std::endl;
            // }
            // //assert(a.z() != b.z());
            return a.z() < b.z();
          }
