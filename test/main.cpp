...
// void loadVs(const std::string &filename, Eigen::Matrix<float, 3, Eigen::Dynamic> &fvs)
// {
...
//     const aiScene *a_scene = importer.ReadFile(file_path.c_str(), aiProcessPreset_TargetRealtime_Fast);
//     if (!a_scene) {
...
// Eigen::Matrix<int, 7, 1> EVector7i(int a, int b, int c, int d, int e, int f, int g)
// {
...
//     std::cout << "file_path:" << file_path << std::endl;
//     Assimp::Importer importer;
//     const aiScene *a_scene = importer.ReadFile(file_path.c_str(), 0); //aiProcessPreset_TargetRealtime_Fast);
//     if (!a_scene) {
//       std::cerr << "Error: " << importer.GetErrorString() << std::endl;
//       return 1;
//     }
...
//       // std::cout << tmp_mesh->mNumVertices <<  " -- " << 3*tmp_mesh->mNumFaces
//       // << std::endl;
...
//         tmp_fvs[vi] = Eigen::Vector3f(vertices[vi].x, vertices[vi].y, vertices[vi].z);
//         if(vi % 3 == 0)
//           keys[vi/3] = Eigen::Vector4i(uvs[vi].x * 1000, uvs[vi].y * 1000, vertices[vi].x * 1000, vi/3);
//         else
//           keys[vi/3].head(3) += Eigen::Vector3i(uvs[vi].x * 1000, uvs[vi].y * 1000, vertices[vi].x * 1000);
//       }

//       std::sort(keys.begin(), keys.end(), [mesh_i](const Eigen::Vector4i &a, const Eigen::Vector4i &b) {
//         if(a.x() == b.x())
//         {
//           if(a.y() == b.y())
//           {
...
//         keys_ofs << (keys[fi] + Eigen::Vector4i(0, 0, 0, vi_offset/3)).transpose() << "\n";
//       }
...
//       exportVtk("debug/mesh_part_" + std::to_string(mesh_i) + ".vtk", cur_fvs.block(0, vi_offset, 3, tmp_mesh->mNumVertices));
...
//     //           cur_fvs.block(0, a_meshes[1]->mNumVertices, 3, a_meshes[1]->mNumVertices));
//     // exportVtk("debug/testm_" + std::to_string(file_id) + ".vtk",
//     //           (fvs_0 + cur_fvs).block(0, a_meshes[0]->mNumVertices, 3, a_meshes[1]->mNumVertices) * 0.5);
...
// /**
//  * have a precision problem when export and loading the exported uvs from blender
// */
// void sortBlenderTemplateMesh(const std::string &fbx_file, const std::string &uvs_file, const std::string &fvs_file)
// {
//   Assimp::Importer importer;
...
//     std::sort(keys.begin() + offset, keys.begin() + offset + nfs[mesh_i], [](const Eigen::Vector3i &a, const Eigen::Vector3i &b) {
//       if(a.x() == b.x())
//       {
//         if(a.y() == b.y())
//         {
...
//   if (!ofs.is_open()) {
//     std::cerr << "Error opening file: debug/template_order_fvs.txt" << std::endl;
//     return;
//   }
//   for (auto fi = 0; fi < full_nf * 3; ++fi) {
//     ofs << sorted_fvs(0, fi) << " " << sorted_fvs(1, fi) << " " << sorted_fvs(2, fi) << "\n";
//   }
//   ofs.close();
...
//   //   "/home/wegatron/win-data/workspace/vk_engine/debug/template_f_uvs.txt",
//   //   "/home/wegatron/win-data/workspace/vk_engine/debug/template_fvs.txt");
//   //MergeTest();
...
{
  /* code */
  vk_engine::Light light_ub;
  // std::cout << reinterpret_cast<size_t>(&(light_ub.direction)) - reinterpret_cast<size_t>(&(light_ub.type)) << std::endl;
  // std::cout << reinterpret_cast<size_t>(&(light_ub.color_intensity)) - reinterpret_cast<size_t>(&(light_ub.direction)) << std::endl;
  // std::cout << reinterpret_cast<size_t>(&(light_ub.position_falloff)) - reinterpret_cast<size_t>(&(light_ub.color_intensity)) << std::endl;
