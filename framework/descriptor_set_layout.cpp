...
namespace vk_engine
{
    VkDescriptorType find_descriptor_type(ShaderResourceType resource_type, bool dynamic)
    {
        switch (resource_type)
        {
            ...
            case ShaderResourceType::Image:
                // an image that can be used in conjunction with a sampler to provide filtered data to a shader.
                return VK_DESCRIPTOR_TYPE_SAMPLED_IMAGE;
                break;
            case ShaderResourceType::ImageSampler:
                // a sampler and an image paired together, can be more efficient on some architectures
            ...
            case ShaderResourceType::BufferUniform:
                // dynamic, include an offset and size that are passed when the descriptor set is bound to the pipeline
                // can use the same descriptor set for multiple objects with different offsets
                // refer to: https://github.com/SaschaWillems/Vulkan/blob/master/examples/dynamicuniformbuffer/README.md
                if (dynamic)
                {
        ...
        const std::vector<ShaderResource> &resource_set)
    {
        for(auto &resource : resource_set)
        {
            ...
            else binding_flags_.push_back(0);
        ...
        if(std::find_if(resource_set.begin(), resource_set.end(), 
            [](const ShaderResource &resource) { 
                return resource.mode == ShaderResourceMode::UpdateAfterBind; }) != resource_set.end())
        {
            ...
            binding_flags_info.bindingCount = static_cast<uint32_t>(binding_flags_.size());            
        ...
        VkResult result = vkCreateDescriptorSetLayout(driver->getDevice(), &layout_info, nullptr, &handle_);
