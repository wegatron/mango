...
GraphicsPipeline::GraphicsPipeline(
  ...
  pipeline_state_->getInputAssemblyState().getCreateInfo(
        input_assembly_state);
  pipeline_info.pInputAssemblyState = &input_assembly_state;
    
  VkPipelineViewportStateCreateInfo viewport_state{VK_STRUCTURE_TYPE_PIPELINE_VIEWPORT_STATE_CREATE_INFO};
  viewport_state.viewportCount = pipeline_state_->getViewportState().viewport_count;
  viewport_state.scissorCount = pipeline_state_->getViewportState().scissor_count;
...
//   pipeline_info.basePipelineIndex = -1;
  ...
  if (vkCreateGraphicsPipelines(driver->getDevice(), cache->getPipelineCache(), 1,
